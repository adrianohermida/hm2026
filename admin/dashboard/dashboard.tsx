...
export const Dashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'leads' | 'processos' | 'faturas' | 'tickets' | 'publicacoes' | 'ia' | 'chatbot' | 'balcao' | 'config'>('overview');
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        else if (activeTab === 'config') endpoint = '/api/admin/stripe/status';

        if (endpoint) {
          const res = await fetch(endpoint);
          const result = await res.json();
          if (activeTab === 'config') setStripeStatus(result);
          ...
          <div className="flex-1 min-w-0 space-y-6">
            ...
            {loading ? (
              ...
              <div className="animate-fade-in">
                ...
                {activeTab === 'config' && <ConfigModule status={stripeStatus} onUpdate={handleConfigUpdate} />}
                {activeTab === 'agenda' && (
                  <div className="space-y-6 animate-fade-in">
                    <div className="flex justify-between items-center">
                      <h2 className="text-2xl font-bold text-white">Gestão de Agenda</h2>
                      <div className="flex gap-2">
                        <button className="bg-brand-primary text-white px-4 py-2 rounded-lg text-sm font-bold">Novo Horário</button>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {appointments.length === 0 ? (
                          ...
                          appointments.map((app) => (
                            <div key={app.id} className="bg-brand-elevated p-6 rounded-2xl border border-white/5 flex flex-col md:flex-row justify-between gap-6">
                              <div className="flex gap-4">
                                <div className="bg-brand-primary/10 p-3 rounded-xl h-fit">
                                  <CalendarIcon className="text-brand-primary" size={24} />
                                </div>
                                <div>
                                  <h3 className="font-bold text-white">{app.form_data.name}</h3>
                                  <p className="text-sm text-white/50">{app.form_data.email} • {app.form_data.phone}</p>
                                  <div className="flex items-center gap-4 mt-2">
                                    <span className="text-xs bg-white/5 px-2 py-1 rounded text-white/70 flex items-center gap-1">
                                      <Clock size={12} /> {app.form_data.appointment_date} às {app.form_data.appointment_time}
                                    </span>
                                    <span className="text-xs bg-brand-primary/10 px-2 py-1 rounded text-brand-primary flex items-center gap-1">
                                      <UserIcon size={12} /> {app.profissional_nome || 'Não atribuído'}
                                    </span>
                                  </div>
                                </div>
                              </div>
                              <div className="flex items-center gap-3">
                                <span className={clsx(
                                  ...
                                  {app.status === 'pending' ? 'Pendente' : app.status === 'approved' ? 'Aprovado' : 'Cancelado'}
                                </span>
...
const ConfigModule = ({ status, onUpdate }: { status: any; onUpdate?: (type: string, value: any) => void }) => {
        ...
        <div className="relative z-10">
          ...
          <div className="grid sm:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-brand-dark/50 p-6 rounded-2xl border border-white/5">
              ...
              <span className="text-lg font-bold uppercase text-white">{status?.mode === 'test' ? '🧪 Test' : status?.mode === 'live' ? '🚀 Live' : 'Desconhecido'}</span>
            </div>
          </div>
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                {status?.connectId && (
                  <div className="text-xs text-green-300 px-3 py-2 bg-green-500/20 rounded-xl">
                    Modo Connect: <strong>{status.connectId.slice(0, 8)}...</strong>
                  </div>
                )}
              </div>
              {testResult && (
      ...
      <div className="bg-brand-elevated p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
        ...
        <div className="grid gap-4">
          {[
            ...
            { label: 'Status Stripe', value: status?.isConfigured ? '✅ Conectado' : '⚠️ Desconectado' },
          ].map((item, i) => (
