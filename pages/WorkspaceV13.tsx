...
export const WorkspaceV13: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  return (
      ...
      <Container className="relative z-10 text-center max-w-2xl animate-in fade-in zoom-in duration-1000">
        <div className="relative inline-block mb-12">
          <div className="w-28 h-28 bg-white rounded-[3rem] shadow-[0_30px_60px_rgba(0,0,0,0.08)] flex items-center justify-center mx-auto border border-slate-50 transition-all duration-700 hover:scale-110">
            <Layout size={48} className="text-brand-secondary opacity-30" />
          </div>
        ...
        <div className="space-y-6 mb-16">
          <Typography variant="h1" font="serif" className="text-brand-primary text-5xl md:text-7xl tracking-tighter leading-[0.9]">
            Espaço <span className="text-brand-secondary italic">Soberano</span>
          </Typography>
          
          <Typography variant="body" className="text-slate-400 text-lg leading-relaxed max-w-md mx-auto font-medium">
            Ambiente de trabalho neutro orquestrado pelo Kernel V13. Pronto para a síntese de novos fluxos estratégicos no domínio <span className="text-brand-primary font-bold">hermidamaia.adv.br</span>.
          </Typography>
        </div>
        ...
        <div className="mt-32 pt-12 border-t border-slate-100 flex flex-col items-center gap-4 opacity-10">
           <div className="flex items-center gap-3">
              <span className="text-[10px] font-black uppercase tracking-[0.6em] text-brand-primary">HM-SOVEREIGN-WORKSPACE</span>
           </div>
           <span className="text-[8px] font-black uppercase tracking-widest italic">Protocolo V13.20.0</span>
        </div>
      </Container>
    </div>
  );
