...
export const LabPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 font-sans select-none relative overflow-hidden">
       ...
       <Container className="text-center max-w-xl animate-in fade-in zoom-in duration-700 relative z-10">
          <div className="w-24 h-24 bg-white rounded-[2.5rem] shadow-xl flex items-center justify-center mx-auto mb-8 border border-slate-100 relative group hover:scale-110 transition-transform">
             <Beaker size={32} className="text-brand-secondary" />
          ...
          <Typography variant="h2" font="serif" className="text-brand-primary mb-4 text-4xl">
            Página <span className="text-brand-secondary italic">Em Branco</span>
          </Typography>
          
          <Typography variant="body" className="text-slate-400 mb-10 text-sm leading-relaxed max-w-sm mx-auto">
            Este é o seu novo Laboratório. Um espaço livre para prototipagem e desenvolvimento de novos módulos seguindo o rigor do <strong>Kernel V12</strong>.
          </Typography>
          ...
          <div className="mt-20 pt-8 border-t border-slate-200/50 flex flex-col items-center gap-2 opacity-30">
             <span className="text-[10px] font-black uppercase tracking-[0.4em] text-brand-primary">HM-LAB-SPACE</span>
             <span className="text-[8px] font-mono text-slate-400">Secure SandBox v1.0</span>
          </div>
       </Container>
    </div>
  );
